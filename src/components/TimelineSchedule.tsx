            ...
            <React.Fragment key={pt.taskId}>
              ...
              >
                ...
                <button
                  ...
                  className="absolute left-3.5 top-5 z-10 w-6 h-6 rounded-full bg-slate-950 border border-slate-700 flex items-center justify-center hover:scale-110 transition-transform cursor-pointer"
                  title={isCompleted ? "Mark incomplete" : "Mark completed"}
                >
                  {isCompleted ? (
                    <CheckCircle2 className="w-5 h-5 text-emerald-400 fill-emerald-500/20" />
                  ) : (
                    <span className="text-xs font-bold text-indigo-400">{pt.order}</span>
                  )}
                </button>
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap sm:flex-nowrap">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center flex-wrap gap-2 mb-1.5">
                      <span className="inline-flex items-center gap-1 text-xs font-bold text-indigo-300 bg-indigo-950/80 border border-indigo-800/80 px-2.5 py-0.5 rounded-lg">
                      ...
                      <span className="text-[10px] font-semibold text-slate-400 bg-slate-950 px-2 py-0.5 rounded-md border border-slate-800">
                        Priority Score: <strong className="text-white">{pt.priorityScore}/100</strong>
                      </span>
                    </div>

                    <h4 className={`text-sm font-bold text-white mb-1 ${isCompleted ? 'line-through text-slate-400' : ''}`}>
                      {originalTask.title}
                    </h4>

                    {/* Intuitive reasoning note */}
                    <p className="text-xs text-slate-300 bg-slate-950/80 p-2.5 rounded-lg border border-slate-800/80 my-2 leading-relaxed">
                      <strong className="text-indigo-400 font-semibold">AI Rationale: </strong>
                      {pt.reasoning}
                    </p>

                    {/* Subtasks or risk warning */}
                    {pt.riskWarning && (
                      <div className="mt-2 inline-flex items-center gap-1.5 text-xs text-amber-300 bg-amber-500/10 px-2.5 py-1 rounded-lg border border-amber-500/20">
